...
FileBackend.prototype.getTile = function(z, x, y, callback) {
    var filepath = this.getPath(z, x, y, this.filetype);
    var self = this;
    var fd = fs.open(filepath, 'r',  function(err, fd) {
        if (err) {
            return callback(err);            
        } 
        fs.fstat(fd, function(err, stats) {
            ...
            fs.readFile(fd, function(error, data) {
                ...
                headers['ETag'] = data.length + '-' + Number(stats.mtime);
                headers['Content-Type'] = 'image/'+self.filetype;                
...
FileBackend.prototype.getGrid = function(z, x, y, callback) {
    var filepath = this.getPath(z, x, y, 'json');
    var self = this;
    var fd = fs.open(filepath, 'r',  function(err, fd) {
        if (err) {
            return callback(err);            
